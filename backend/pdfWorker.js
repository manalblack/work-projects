    ...
    for(const item of cart) {
       ...
       imageBuffer = await createImageBuffer(image);
        ...
        for(let i = 0; i < quantity; i++) {
            ...
            // Adding the pdf file to the bucket/ 
            const {data: uploadedData, error: uploadError} = await supabase.storage.from('tickets_qr_codes').upload(`ticket_${ticketId}.pdf`, pdfStream, {
